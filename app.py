...
with tab2:
    st.subheader("🎤 Spraak naar tekst (Demo)")
    audio_file = st.file_uploader("Upload WAV/MP3/FLAC voor transcriptie", type=["wav", "mp3", "flac"])
    if audio_file:
...
"""
    ...
    with st.spinner("AI analyseert intake…"):
        try:
            ...
            st.markdown(f"### {emoji} Urcientie: **{urgency}**")
            st.text(output)
